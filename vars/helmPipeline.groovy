...
void call(Closure body) {
    ...
    imageWithTag = "${Docker.PUBLIC_DOCKER_REGISTRY_USER}/${JOB_NAME.split('/')[0]}:${tag}" // Image name
    ...
    pipeline {
        ...
        environment {
            ...
            BASE_JOB_NAME = "${JOB_NAME.split('/')[0]}" // Extract the base job name
            JOB_BRANCH = "${JOB_NAME.split('/')[1]}" // Extract the branch name from the job name
        ...
        //     // Dynamically populate parameters using the shared library
        //     choice(name: 'TAG', choices: parametersHandler.getTags(), description: 'Select a tag')
